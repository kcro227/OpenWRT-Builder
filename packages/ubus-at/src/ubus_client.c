...
int ubus_client_init(ubus_client_t *client) {
    ...
    if (!client->ctx) {
        printf("[ERR] Failed to connect to ubus");
        return -1;
    }
    
    client->connected = 1;
    printf("[DEBUG] Connected to ubus successfully");
    return 0;
}
...
void ubus_client_cleanup(ubus_client_t *client) {
    if (client && client->ctx) {
        ...
        client->ctx = NULL;
        client->connected = 0;
        printf("[DEBUG] Disconnected from ubus");
    }
}
...
static uint32_t find_service_id(struct ubus_context *ctx, const char *service_name) {
    ...
    if (ret != UBUS_STATUS_OK) {
        printf("[ERR] Failed to find service %s: %s", service_name, ubus_strerror(ret));
        return 0;
    }
    return id;
}
...
int ubus_at_open_device(ubus_client_t *client, const char *device_path, 
    ...
    if (ret != UBUS_STATUS_OK) {
        printf("[ERR] Failed to open AT device via ubus: %s", ubus_strerror(ret));
        return -1;
    }
    
    printf("[DEBUG] Opened AT device %s via ubus", device_path);
    return 0;
}
...
int ubus_at_close_device(ubus_client_t *client, const char *device_path) {
    ...
    if (ret != UBUS_STATUS_OK) {
        printf("[ERR] Failed to close AT device via ubus: %s", ubus_strerror(ret));
        return -1;
    }
    
    printf("[DEBUG] Closed AT device %s via ubus", device_path);
    return 0;
}
...
int ubus_send_at_command(ubus_client_t *client, const char *device_path,
    ...
    if (ret != UBUS_STATUS_OK) {
        printf("[ERR] Failed to send AT command via ubus: %s", ubus_strerror(ret));
        return -1;
    }
    
    printf("[DEBUG] Sent AT command via ubus: %s", at_cmd);
...
int ubus_send_at_command_only(ubus_client_t *client, const char *device_path,
    ...
    if (ret != UBUS_STATUS_OK) {
        printf("[ERR] Failed to send AT command (sendonly) via ubus: %s", ubus_strerror(ret));
        return -1;
    }
    
    printf("[DEBUG] Sent AT command (sendonly) via ubus: %s", at_cmd);
    return 0;
}
